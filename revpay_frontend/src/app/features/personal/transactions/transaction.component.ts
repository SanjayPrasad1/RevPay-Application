...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, CommonModule, ReactiveFormsModule, ExportTransactionsComponent],
  template: `
    <div class="page">
      ...
      <app-export-transactions></app-export-transactions>
    </div>
  `,
  styles: [`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tx-list { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06);
      border-radius: 16px; overflow: hidden; }

    .tx-item { display: flex; align-items: center; gap: 1rem; padding: 1.1rem 1.25rem;
    ...
    .tx-status { font-size: 0.7rem; padding: 0.15rem 0.5rem; border-radius: 999px;
    ...
    .empty-state { padding: 3rem; text-align: center; color: #475569; font-size: 0.9rem; }

    .pagination { display: flex; align-items: center; justify-content: center;
      gap: 1.5rem; margin-top: 1.5rem; }

    .page-btn { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1);
      color: #94a3b8; padding: 0.5rem 1.25rem; border-radius: 10px; cursor: pointer;
...
export class TransactionsComponent implements OnInit {
  ...
  // myAccountId = signal<number | null>(null);

  ngOnInit() {
    // const user = this.authService.currentUser();
    // if (user) this.myAccountId.set(user.primaryAccountId);
    this.load();
  }
